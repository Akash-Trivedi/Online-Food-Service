...
function loginForm(){
  echo '<form action="sendotp.php" method="POST" class="form-inline d-flex justify-content-center md-form form-sm active-purple-2 mt-2>"';
  if(isset($_SESSION['otp'])){
    echo '<label for="otp"> OTP: </label><input type="text" id="otp" name="otp"  class="form-control form-control-sm">';
    echo '<input type="submit" value="Login" name="cnfotp">';
  } else{
    echo '<label for="contact">Contact: </label><input type="text" id="contact" placeholder="Number" name="contact" class="form-control form-control-sm mr-3 w-20">';
